  ...
  function addDecodedCard() {
    ...
    article.innerHTML = `
      ...
      <div class="archive-card__details">
        <p class="archive-card__details-text">"База «Сектор-7». Прием. Фиксируем нестабильный сигнал неизвестного происхождения. Частота не закрепляется - уходит каждые несколько секунд. Структура импульсов не соответствует ни одному из известных протоколов.

Пытаемся синхронизироваться. Есть повторяющийся фрагмент, возможно - искажённая передача или навигационный маркер. Источник определить не удаётся: сигнал как будто смещается в пространстве.

Продолжаем запись и анализ. Если кто-то принимает - подтвердите. Сектор-7, конец связи... пока."</p>
      </div>
    `;
